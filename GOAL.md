...
current_run_id: sprint-02-run-2
...
> 4. Reviews -- 3-seat panel per charters; ownership-matrix sweep of the DB-backed Job resource (collection filters, nested-id FKs in bodies, cross-resource reads -- not just id-addressed 404s); Codex D1 pre-guard (fresh block -- MUST trigger); Codex D2 pre-merge (exemplar first-instance -- MUST trigger). Ledger closed per process spec section 4 dispositions. (run-2 note: the shared mock Application/searches/resume/shortlist layer stays un-tenanted until sprint-04 -- DEBT-5, PO-accepted; the tenancy conjunct covers the resource being DB-converted, not the whole mock surface.)
