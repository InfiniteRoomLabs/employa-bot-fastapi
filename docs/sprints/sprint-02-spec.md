...
- PIN-2 (inbox stays mock): getJobsInbox stays mock-served with an explicit manifest note. Grounds: JobInboxItem is a search-feed projection keyed by mock search ids; searches are out of Release 0.1 scope; DB-backing the inbox would create the forbidden DB->mock dependency. The Done-when's "jobs contract operations ... flipped" is read per the Goal block's own "(ownership manifest flips per op)" clause: getJobs + getJob flip to implemented.
...
- PIN-8 (journey discriminators): the wizard's review-step company/role fields become real inputs; core-journey creates a job with a per-run unique company string, asserts it absent before creation, present after, and present after reload; it captures the created jobId from the createApplication response and asserts /jobs/{jobId} renders (DB-backed getJob). DB provenance is carried by AC-02's discriminator tests (below), not by the browser alone.
- PIN-9 (frontend de-mock scope): the existing add-app review step's fixed display fields (company, role, location) become controlled inputs prefilled with the current fixture values. No new screens, no route changes, no contract change. This is de-mocking the existing manual-capture flow, not new product surface.
...
| AC-04 | browser create -> persists + lists after reload | core-journey transcript per PIN-8 against the compose stack |
...
| S02-D1-2 | HIGH | fixed | PIN-2: per-op decision recorded openly (per the Goal block's "flips per op" clause); provenance discriminators in AC-02 prevent silent mock-backing of the flipped ops; inbox exclusion grounded in the abandonment-safety rule |
...
- P4: wizard editable fields (PIN-9) + frontend/e2e/core-journey.spec.ts (PIN-8) + workflow check. Commit boundary.
