...
| AC-02 | composite FK rejects cross-tenant job_id at the DB, incl. under app_runtime | as owner AND under SET ROLE app_runtime + the caller's GUC (D2-2 backstop): insert shortlist_entry with (own user_id, a job_id owned by ANOTHER tenant) -> composite-FK rejection; with own job_id -> accepted; NULL job_id -> accepted (FK skipped) |
| AC-02b | dedup UNIQUE(user_id, job_id) partial, concurrency-safe | TWO CONCURRENT addToShortlist requests through the route (two threads, one shared client) for the SAME (user_id, job_id) -> EXACTLY one 201 and one 409, and a DB assert of exactly one row (the losing concurrent request maps to the 409 conflict envelope). Backed by a deterministic two-connection SQL-layer test (A holds the key open; B under lock_timeout blocks and raises -> real contention). Plus: two NULL-job rows both accepted |
