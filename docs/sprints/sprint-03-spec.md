...
| AC-02b | dedup UNIQUE(user_id, job_id) partial, concurrency-safe | the two-connection race (D2-1, deterministic): connection A inserts the key and holds its txn open; connection B under a short lock_timeout tries the SAME key while A is open and is BLOCKED -> raises (real contention, not sequential); A commits -> exactly one row. Plus: two NULL-job rows both accepted; a duplicate through the route -> 409 conflict envelope |
