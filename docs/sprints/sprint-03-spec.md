...
| AC-02b | dedup UNIQUE(user_id, job_id) partial, concurrency-safe | TWO concurrent authenticated addToShortlist requests for the SAME (user_id, job_id) on separate connections -> EXACTLY one 201 and one 409, then a DB assert of exactly one row (the two-connection race, foreshadowing sprint-04); plus: two NULL-job rows both accepted; a sequential duplicate maps to the 409 conflict envelope |
