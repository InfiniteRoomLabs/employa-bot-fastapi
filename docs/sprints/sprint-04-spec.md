...
- PIN-15 (snapshot semantics; text aligned with the 3b design at S6 -- SIM-1): resume_snapshot is UNIQUE(user_id, application_id) (the mock keys snapshots by application id); getResumeSnapshot = 404 unknown/removed id, 409 conflict for stage in (saved, drafting), else the REAL row when one exists. Seed materializes a real snapshot row for every seeded application past drafting, and reactivated applications reuse the one from their original applied transition. For the runtime-legal paths that cross into stage-past-drafting WITHOUT ever passing applied (markWon straight from saved/drafting/screening), the route keeps the mock's deterministic read-only synthesis (uuid5, no storage) -- breaking those reads with a 500 would punish a legal lifecycle path. The journey's discriminating assertion (snapshot id == submittedSnapshotId) proves the real-materialization path is what serves actual applied transitions.
