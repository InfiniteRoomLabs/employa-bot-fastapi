...
- Last verified checkpoint: 3a COMPLETE (this commit) -- five-table migration (dca1119) + resume vertical (8 ops) + applications/archive vertical (5 ops) DB-backed, 13 manifest flips, full suite 421 green, lint clean. Codex D1 fired (thread 019f6166, UNSOUND -> spec hardened PIN-1..19 -> all 10 CLOSED, SOUND at b640011). Packet reports + running ledger in the session scratchpad; findings fold into the ship ledger.
- Exact next action: 3b -- second migration creating application_stage_transition() + application_soft_remove() + delete_user_with_history() per the spec's "3b design" section (draft SQL in scratchpad/3b-function-draft.sql); convert the four PIN-11 user-delete call sites; then transitionApplication + getResumeSnapshot routes; AC-04a/b/c + AC-05 two-connection tests (PR-7 connection hygiene).
