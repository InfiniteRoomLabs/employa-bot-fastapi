...
### sprint-04-applications-resume-snapshot (run sprint-04-run-1, S7 in progress; branch sprint-04-apps-resume-snapshot, pre-merge HEAD dcc5c56)
...
5. Undo = compensating transition: undoMarkWon claims the persistent undo_grant atomically (single UPDATE on statement_timestamp, PIN-4; window from Settings.UNDO_WINDOW_SECONDS -- DEBT-3 closed) and appends source=user_correction + corrects_transition_id with the corrected row STILL PRESENT (AC-06a); double-undo two-connection race -> exactly one winner; expired -> 409 undo_window_expired; unknown/foreign token byte-identical 404s.
6. Ownership matrix: intruder tests across every flipped op + nested ids (transition resumeId, fork jobId, undo token, snapshot/timeline/archive) all tenant-indistinguishable 404s; QA panel seat independently attacked over LIVE HTTP with two real users incl. a leaked unexpired undo token -- ZERO findings; PIN-19 matrix + FORCE RLS introspected on all 5 tables.
...
|---|---|---|---|---|---|
| S04-D1-1..10 | Codex D1 (thread 019f6166) | 5 HIGH / 5 MED | plan-attack findings on the manifest/evidence (APPLIED atomicity, mutation-function privilege boundary, undo objectivity, undo_grant persistence, default-swap ordering, op-set boundary, TRUNCATE, migration paths, DB-discrimination, ledger scope) | 9 fixed + 1 recorded (process-defined) | PIN-1..19 + AC matrix in sprint-04-spec.md; D1 reply round: all 10 CLOSED, verdict SOUND |
...
| RV-3 | resume-verify | LOW | fk_resume_snapshot_resume backstop untested at 3a | fixed (3b) | applied-lock test asserts deleteResume 409 once the snapshot exists (test_transitions.py) |
| RV-4 | resume-verify | LOW | getResumes updated-DESC id tie-break untested under timestamp collision | waived-by-default (trivial; deterministic order asserted, collision case cosmetic) -- flagged for Wes's S0 review | ledger row; revisit only if ordering ever regresses |
...
| IMPL-BUG-1/2 | implementers (self-caught) | MED-class | SQLAlchemy flush-order vs uq_resume_user_default; raw-SQL composite FKs invisible to ORM insert ordering | fixed in-packet | explicit session.flush() at both sites; retro PR-9 |
...
| LCV-3 | lc-verify2 | INFO | 3 orphaned dismissed rows in the DEV DB (crash-interrupted race-test teardown residue, predates the commit) | resolved (environmental) | clean-volume S5 reset wiped them; PIN-15 verified holding for everything the commit seeds |
| LCV-4 | lc-verify2 | INFO | DEFAULT resume used_in=12 vs 7 hops | disproved-as-defect | mock-parity baseline 5 + 7 seeded hops = 12 |
