...
- Last verified checkpoint: 3b COMPLETE (this commit) -- the ONE stage-mutation function (migration 7a2c91d40e88: application_stage_transition + application_soft_remove + delete_user_with_history), stage_flow.py SQLSTATE->envelope driver, transitionApplication + getResumeSnapshot DB-backed (15/20 ops flipped), PIN-11 wired through all four user-delete paths, AC-04a/b/c + AC-05 proven (route-level threaded race [200,409] with no orphan children; deterministic SQL-level lock contention under lock_timeout; induced-failure APPLIED rollback). Full suite 443 green, lint clean, races stable 3x. Prior: 3a at a38a1e9/dca1119; D1 SOUND (thread 019f6166, PIN-1..19).
- Exact next action: 3c backend -- work order at scratchpad/3c-backend-work-order.md (markWon/undo/dismiss/reactivate/timeline flips, Settings.UNDO_WINDOW_SECONDS closes DEBT-3, seed extension incl. fixture applications as DB rows per PIN-15, seam removal in getApplication/getResumeSnapshot); then 3c journey extension (frontend e2e); then S5 gates -> S6 panel (charters in sprint-04-spec.md) -> Codex D2 (MUST) -> S7 ship + self-advance to sprint-05.
