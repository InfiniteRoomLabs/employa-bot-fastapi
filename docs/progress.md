...
| RV-1/RV-2 | resume-verify (Sonnet) | INFO | impl-report test count off-by-cosmetic; fk_resume_fork_job dead branch in the delete backstop set | recorded | verifier report; dead branch harmless and kept |
...
| COR-O-1 | panel-correctness | -- | the LEGAL_TRANSITIONS matrix is an app-layer invariant (allowed_from is caller-supplied to the function); DB enforces atomicity/versioning/tenancy/append-only, not the matrix -- the spec's explicit race-safety choice | recorded (by design, spec 3b design section) | panel-correctness.md |
| COR-O-2 | panel-correctness | -- | undo route lacks an EMP22 error_paths entry for a near-impossible race (message degrades, behavior correct) | recorded (cosmetic, no action) | panel-correctness.md |
...
| D2 rows | Codex D2 | -- | recorded on dispatch below | -- | -- |
