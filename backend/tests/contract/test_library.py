...
def test_get_contacts_lists_seeded_three(client: TestClient) -> None:
...
def test_get_contact_returns_wire_shape(client: TestClient) -> None:
...
def test_get_contact_unknown_id_returns_404_envelope(client: TestClient) -> None:
    resp = client.get(f"/api/v1/contacts/{UNKNOWN_ID}")
...
def test_create_contact_persists_and_returns_201(client: TestClient) -> None:
    resp = client.post("/api/v1/contacts", json=_contact_draft())
    assert resp.status_code == 201
    body = resp.json()
    assert body["name"] == "New Contact"
    assert "deletedAt" not in body or body["deletedAt"] is None

    listing = client.get("/api/v1/contacts").json()
    assert len(listing) == 4
    assert client.get(f"/api/v1/contacts/{body['id']}").status_code == 200


def test_update_contact_replaces_fields_and_bumps_updated(client: TestClient) -> None:
    before = client.get(f"/api/v1/contacts/{CONTACT_ID}").json()
    resp = client.patch(
...
def test_update_contact_unknown_id_404(client: TestClient) -> None:
    resp = client.patch(f"/api/v1/contacts/{UNKNOWN_ID}", json=_contact_draft())
    assert resp.status_code == 404
    assert resp.json()["kind"] == "not_found"


def test_delete_contact_soft_deletes_and_excludes_from_list(client: TestClient) -> None:
    ...
    direct = client.get(f"/api/v1/contacts/{CONTACT_ID}")
    assert direct.status_code == 200
    assert direct.json()["deletedAt"] is not None


def test_delete_contact_unknown_id_404(client: TestClient) -> None:
    resp = client.delete(f"/api/v1/contacts/{UNKNOWN_ID}")
    assert resp.status_code == 404
...
def test_get_accomplishments_lists_seeded_three(client: TestClient) -> None:
    resp = client.get("/api/v1/accomplishments")
    assert resp.status_code == 200
    body = resp.json()
    assert len(body) == 3
...
def test_create_accomplishment_defaults_used_in_zero(client: TestClient) -> None:
    resp = client.post("/api/v1/accomplishments", json=_accomplishment_draft())
    assert resp.status_code == 201
    body = resp.json()
    assert body["usedIn"] == 0
    assert body["source"] is None

    listing = client.get("/api/v1/accomplishments").json()
    assert len(listing) == 4


def test_update_accomplishment_replaces_fields(client: TestClient) -> None:
    resp = client.patch(
...
def test_update_accomplishment_unknown_id_404(client: TestClient) -> None:
    resp = client.patch(
...
def test_delete_accomplishment_soft_deletes_and_excludes_from_list(
    ...
    resp = client.delete(f"/api/v1/accomplishments/{ACCOMPLISHMENT_ID}")
    assert resp.status_code == 204
    listing = client.get("/api/v1/accomplishments").json()
    assert ACCOMPLISHMENT_ID not in {a["id"] for a in listing}


def test_delete_accomplishment_unknown_id_404(client: TestClient) -> None:
    resp = client.delete(f"/api/v1/accomplishments/{UNKNOWN_ID}")
    assert resp.status_code == 404
...
def test_derive_accomplishment_from_project_snapshots_and_backlinks(
    ...
    project = client.get("/api/v1/projects").json()
    ...
    still_derived = next(
        a
        for a in client.get("/api/v1/accomplishments").json()
        if a["id"] == derived["id"]
    )
...
def test_get_answers_lists_seeded_four(client: TestClient) -> None:
    resp = client.get("/api/v1/answers")
    assert resp.status_code == 200
    body = resp.json()
    assert len(body) == 4
...
def test_create_answer_persists(client: TestClient) -> None:
    resp = client.post("/api/v1/answers", json=_answer_draft())
    assert resp.status_code == 201
    listing = client.get("/api/v1/answers").json()
    assert len(listing) == 5


def test_update_answer_replaces_fields(client: TestClient) -> None:
    resp = client.patch(
...
def test_update_answer_unknown_id_404(client: TestClient) -> None:
    resp = client.patch(f"/api/v1/answers/{UNKNOWN_ID}", json=_answer_draft())
    assert resp.status_code == 404


def test_delete_answer_soft_deletes_and_excludes_from_list(client: TestClient) -> None:
    resp = client.delete(f"/api/v1/answers/{ANSWER_ID}")
    assert resp.status_code == 204
    listing = client.get("/api/v1/answers").json()
    assert ANSWER_ID not in {a["id"] for a in listing}


def test_delete_answer_unknown_id_404(client: TestClient) -> None:
    resp = client.delete(f"/api/v1/answers/{UNKNOWN_ID}")
    assert resp.status_code == 404
...
def test_get_projects_lists_seeded_two(client: TestClient) -> None:
...
def test_create_project_persists(client: TestClient) -> None:
    resp = client.post("/api/v1/projects", json=_project_draft())
    assert resp.status_code == 201
...
def test_update_project_unknown_id_404(client: TestClient) -> None:
    resp = client.patch(f"/api/v1/projects/{UNKNOWN_ID}", json=_project_draft())
    assert resp.status_code == 404


def test_delete_project_soft_deletes_and_excludes_from_list(client: TestClient) -> None:
...
def test_delete_project_unknown_id_404(client: TestClient) -> None:
    resp = client.delete(f"/api/v1/projects/{UNKNOWN_ID}")
    assert resp.status_code == 404
...
def test_get_credentials_empty_seed(client: TestClient) -> None:
...
def test_trash_is_empty_before_any_delete(client: TestClient) -> None:
    resp = client.get("/api/v1/library/trash")
    assert resp.status_code == 200
    assert resp.json() == []


def test_trash_lists_soft_deleted_across_kinds_newest_first(client: TestClient) -> None:
...
def test_restore_library_item_clears_deleted_at_and_round_trips(
    ...
    client.delete(f"/api/v1/contacts/{CONTACT_ID}")
    assert CONTACT_ID not in {c["id"] for c in client.get("/api/v1/contacts").json()}

    resp = client.post(f"/api/v1/library/contact/{CONTACT_ID}/restore")
    assert resp.status_code == 204
...
def test_restore_library_item_unknown_id_404(client: TestClient) -> None:
    resp = client.post(f"/api/v1/library/contact/{UNKNOWN_ID}/restore")
    assert resp.status_code == 404
    assert resp.json()["kind"] == "not_found"


def test_purge_library_item_is_permanent(client: TestClient) -> None:
    client.delete(f"/api/v1/contacts/{CONTACT_ID}")

    resp = client.delete(f"/api/v1/library/contact/{CONTACT_ID}/purge")
    assert resp.status_code == 204
    ...
    assert (
        client.post(f"/api/v1/library/contact/{CONTACT_ID}/restore").status_code == 404
    )


def test_purge_library_item_unknown_id_404(client: TestClient) -> None:
    resp = client.delete(f"/api/v1/library/contact/{UNKNOWN_ID}/purge")
    assert resp.status_code == 404
...
def test_get_deletion_impact_project_counts_live_accomplishment_dependents(
    ...
    resp = client.get(f"/api/v1/library/project/{PROJECT_ID}/deletion-impact")
    assert resp.status_code == 200
    body = resp.json()
...
def test_get_deletion_impact_project_excludes_soft_deleted_dependents(
    ...
    client.delete(f"/api/v1/accomplishments/{tooling_accomplishment_id}")

    resp = client.get(f"/api/v1/library/project/{PROJECT_ID_TOOLING}/deletion-impact")
    assert resp.status_code == 200
    body = resp.json()
    assert body["total"] == 0
...
def test_get_deletion_impact_non_project_kind_has_no_dependents(
    ...
    resp = client.get(f"/api/v1/library/contact/{CONTACT_ID}/deletion-impact")
    assert resp.status_code == 200
    body = resp.json()
    assert body["total"] == 0
    assert body["dependents"] == []
...
def test_get_deletion_impact_unknown_id_404(client: TestClient) -> None:
    resp = client.get(f"/api/v1/library/project/{UNKNOWN_ID}/deletion-impact")
    assert resp.status_code == 404
    assert resp.json()["kind"] == "not_found"
